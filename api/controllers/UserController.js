...
module.exports = {
  ...
  signup: function (req, res) {
    ...
    //Creating Ether wallet address
    let privateKeyHex = "";
    let possible = "1234567890abcdef";
    for (let i = 0; i < 64; i++) {
      privateKeyHex += possible.charAt(Math.floor(Math.random() * possible.length));
    }
    var privateKey = new Buffer(privateKeyHex, 'hex');

    let eth_wallet = new ETHWallet(privateKey);
    let eth_addr = eth_wallet.address;
